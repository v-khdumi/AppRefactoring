...
export function validatePreparedLocks(snapshot:ArtifactSnapshot,files:PreparedFile[]){
  ...
  for(const file of files){
    if(!/(^|\/)package-lock\.json$/.test(file.path))throw new Error("Only npm-generated package-lock.json artifacts are permitted.");
