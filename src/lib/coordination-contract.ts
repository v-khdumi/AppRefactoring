...
export const coordinationContractSchema=z.object({
  ...
  for(const item of contract.interfaces){
    if(!projects.has(item.provider)||item.consumers.some(consumer=>!projects.has(consumer)||consumer===item.provider))context.addIssue({code:z.ZodIssueCode.custom,message:`Interface ${item.id} must reference distinct declared provider and consumer projects.`});
  }
  if(contract.projects.length>1&&!contract.interfaces.length)context.addIssue({code:z.ZodIssueCode.custom,message:"Multiple projects require explicit integration or file contracts."});
});
